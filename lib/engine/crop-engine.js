...
function evaluateCropIntelligence(farmData, cropsList, weatherData, marketPrices = []) {
  ...
  const targetSeason = farmData?.season || "Kharif";
  let eligibleCrops = cropsList.filter(
    (c) => c.season === targetSeason || c.season === "Year-round" || targetSeason === "Year-round"
  );
  if (eligibleCrops.length === 0) eligibleCrops = cropsList;
