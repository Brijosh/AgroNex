...
export function CustomCropModal({ isOpen, onClose, onCropCreated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-elevated border border-slate-200 space-y-4 my-8">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-1 text-[11px] font-bold text-slate-600 uppercase tracking-wider">
            <Sprout className="w-3.5 h-3.5 text-crop-600" /> Quick Fruit & Crop Presets:
          </div>
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-xs font-semibold border transition-all ${
                  formData.name === preset.name
                    ? "bg-crop-700 text-white border-crop-800 shadow-xs"
        ...
        {error && <p className="text-xs text-rose-600 font-medium bg-rose-50 p-2 rounded-lg">{error}</p>}

        <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
          <div>
            <label className="block font-bold text-slate-700 mb-1">Crop / Fruit Name</label>
            <input
              type="text"
              placeholder="e.g. Dragonfruit, Mango, Strawberry, Avocado..."
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              className="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm font-semibold focus:ring-2 focus:ring-crop-500"
              required
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block font-bold text-slate-700 mb-1">Water Need</label>
              <select
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block font-bold text-slate-700 mb-1">Avg Yield (kg/ha)</label>
              <input
                type="number"
                placeholder="4000"
            ...
            <div>
              <label className="block font-bold text-slate-700 mb-1">Cultivation Cost (₹/ha)</label>
              <input
                type="number"
                placeholder="40000"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block font-bold text-slate-700 mb-1">Market Price (₹/kg)</label>
              <input
                type="number"
                placeholder="30"
          ...
          <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
            <Button type="button" variant="outline" onClick={onClose} size="sm" className="font-bold">Cancel</Button>
            <Button type="submit" variant="primary" disabled={loading} size="sm" className="font-bold">
              {loading ? "Analyzing & Saving..." : "Analyze & Save Crop"}
            </Button>
          </div>
        </form>
      </div>
